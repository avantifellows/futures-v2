"""
Load the state-CET clean parquet from GCS into BigQuery.

WRITE_TRUNCATE, idempotent. Run build_clean.py + upload_to_gcs.py first.

Usage:
  python3 scripts/load_bq.py                                  # load all tables
  python3 scripts/load_bq.py --table state_cet_fact_closing_ranks
...
def main() -> None:
    ...
    print(f"state_cet → {BQ_PROJECT}.external_data_sources.*   ({'dry-run' if args.dry_run else 'load'})")
    for t in chosen:
