"""
Stage the state-CET raw CSV and clean parquet to GCS.

- raw/  → gs://avantifellows-external-data/state_cet/raw/   (provenance)
- clean/→ gs://avantifellows-external-data/state_cet/clean/ (the parquet BQ loads)
...
def main() -> None:
    ...
    print(f"state_cet → gs://{GCS_BUCKET}/{GCS_PREFIX}/   ({'dry-run' if args.dry_run else 'upload'})")
