"""
Stage the MCC raw CSV and clean parquet to GCS.

- raw/  → gs://avantifellows-external-data/mcc/raw/   (provenance)
- clean/→ gs://avantifellows-external-data/mcc/clean/ (the parquet BQ loads)
...
def main() -> None:
    ...
    print(f"mcc → gs://{GCS_BUCKET}/{GCS_PREFIX}/   ({'dry-run' if args.dry_run else 'upload'})")
